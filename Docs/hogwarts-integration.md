...
**The UE 5.8 game and a Hogwarts Legacy mod are separate builds.** The practical Creator Kit route is a single-player arena content mod with Blueprint gameplay and bots. A full networked Hogwarts Legacy match is not a verified capability of the official kit. This repository currently contains port sources; it does not contain a playable or published Hogwarts Legacy Basketbroom mod.
...
Only plain repository files were changed during this investigation. No Creator Kit process was started, installed kit file modified, native HL asset fabricated, or mod uploaded. The next proof is the small 4.27 arena/graph in PIE, followed by cloud-cooked game validation.
