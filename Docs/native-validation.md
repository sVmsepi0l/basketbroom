...
Full regulation remains incomplete, including penalty-shot execution, full spell parity and final officiating/adjudication flows. [Partial native wandplay](native-wandplay-validation.md) now covers selected hits, effects, host dispositions and local replication, with 18 sporting adapters implemented and 13 contextual adapters pending. Portable rule coverage cannot certify every remaining Unreal integration. No complete ruleset, Hogwarts Legacy multiplayer or sustained-performance claim follows from these milestones.
