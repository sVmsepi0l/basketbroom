...
Config legacy_crown_config(Config c=Config{}) { c.enable_legacy_crown_exit=true; return c; }
Match short_match(bool legacy_crown=false) { Config c; c.enable_legacy_crown_exit=legacy_crown; c.quarter_ms=10000; c.overtime_ms=5000; c.snitch_release_ms=5000; return Match(c); }
...
CASES = [
("closed_net_is_default_and_crown_exits_are_atomic_rejections", r'''Match m; CHECK(!m.config.enable_legacy_crown_exit); for(int b=0;b<7;++b){auto before=fingerprint(m); CHECK(!m.crown_exit(b,{0,0,138},1)&&fingerprint(m)==before); CHECK(m.penalties.empty()&&m.balls[b].crown_deadline==-1);} CHECK(m.possess(1,0)); auto before=fingerprint(m); CHECK(!m.crown_exit(0,{0,0,207},1,true)&&fingerprint(m)==before&&m.balls[0].controller==1);'''),
...
("crown_only_kills_one_ball_and_flag_survives_return", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(0,{0,0,138},1)); CHECK(!m.balls[0].live&&m.balls[1].live); m.advance(2000); CHECK(m.crown_return(0)); m.advance(1000); CHECK(m.status==Status::Live&&m.penalties[0].pending); CHECK(m.pause()&&!m.resume()); CHECK(m.resolve_penalty(1,"captain declined restoration after advantage")&&m.resume());'''),
("missed_reentry_target_triggers_safety_stoppage", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(1,{1,2,138})); CHECK(m.advance(10000)==3000&&m.status==Status::Paused); CHECK(m.balls[1].crown_mark[0]==1&&m.balls[1].crown_mark[1]==2&&m.balls[1].crown_mark[2]==138); CHECK(m.resume()&&m.balls[1].live);'''),
("dead_roof_delay_is_moderate", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(5,{0,0,138},5,true)); CHECK(m.penalties[0].severity==Severity::Moderate);'''),
("ordinary_crown_restart_requires_stoppage_and_allows_opposing_chaser", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(0,{2,3,138},1)); CHECK(m.penalties[0].crown_restoration_pending&&!m.prepare_crown_restart(1)); CHECK(m.crown_return(0)&&m.penalties[0].pending&&m.balls[1].live); CHECK(m.pause()&&m.prepare_crown_restart(1)); CHECK(!m.penalties[0].pending&&m.penalties[0].crown_restoration_pending&&m.balls[0].crown_restart_penalty==1); CHECK(!m.restart(0,9)&&m.resume()); CHECK(!m.restart(0,1)&&!m.restart(0,13)&&m.restart(0,9)); CHECK(m.balls[0].controller==9&&m.balls[0].protection_until==3000&&!m.penalties[0].crown_restoration_pending&&m.penalties[0].crown_restoration_receiver==9&&score_is(m,0,0));'''),
("unknown_crown_return_never_creates_restoration", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(1,{4,5,138})&&m.crown_return(1)); CHECK(m.penalties.empty()&&m.pause()&&!m.prepare_crown_restart(1)&&m.resume()&&m.balls[1].live);'''),
("crown_hazard_restoration_is_free_flight_to_hurleyback", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(5,{4,5,138},5)&&m.crown_return(5)&&m.pause()&&m.prepare_crown_restart(1)&&m.resume()); CHECK(!m.restart(5,9)&&m.restart(5,14)); CHECK(m.balls[5].live&&m.balls[5].controller==-1&&!m.penalties[0].crown_restoration_pending&&m.penalties[0].crown_restoration_receiver==14);'''),
("crown_restart_waits_while_all_opponents_unavailable", r'''Config c; c.removal_ms=1000; Match m(legacy_crown_config(c)); CHECK(m.crown_exit(0,{0,0,138},1)&&m.crown_return(0)&&m.pause()); for(int p:{8,9,10,11,12}) {int id=m.record_penalty(p,"unsafe contact",Severity::Serious); CHECK(id>0&&m.resolve_penalty(id,"removal"));} CHECK(m.prepare_crown_restart(1)&&m.resume()); for(int p:{8,9,10,11,12}) CHECK(!m.restart(0,p)); CHECK(!m.balls[0].live&&m.penalties[0].crown_restoration_pending); m.advance(1000); CHECK(m.restart(0,9)&&m.balls[0].controller==9);'''),
("crown_reservation_survives_quarter_resume", r'''auto m=short_match(true); CHECK(m.crown_exit(1,{2,3,138},1)&&m.crown_return(1)); m.advance(10000); CHECK(m.status==Status::QuarterBreak&&m.prepare_crown_restart(1)&&m.resume()&&m.quarter==2); CHECK(m.balls[1].dead_reason=="crown_restart"&&m.balls[1].crown_mark[0]==2&&m.restart(1,9));'''),
("crown_reservation_survives_overtime_transition", r'''auto m=short_match(true); for(int i=0;i<3;++i){m.advance(10000); CHECK(m.resume());} CHECK(m.crown_exit(0,{2,3,138},1)&&m.crown_return(0)); m.advance(10000); CHECK(m.status==Status::Review&&m.prepare_crown_restart(1)&&m.certify()&&m.phase==Phase::Overtime&&m.resume()); CHECK(m.balls[0].dead_reason=="crown_restart"&&m.restart(0,9)&&m.balls[0].controller==9);'''),
("same_ball_crown_restorations_remain_ordered_across_stoppages", r'''Match m(legacy_crown_config()); CHECK(m.crown_exit(0,{1,0,138},1)&&m.crown_return(0)&&m.crown_exit(0,{2,0,138},9)&&m.crown_return(0)&&m.pause()); CHECK(m.prepare_crown_restart(2)&&m.balls[0].crown_restart_penalty==-1&&m.prepare_crown_restart(1)&&m.resume()&&m.restart(0,9)); CHECK(!m.penalties[0].crown_restoration_pending&&m.penalties[1].crown_restoration_pending); CHECK(m.pause()&&m.prepare_crown_restart(2)&&m.resume()&&m.restart(0,1)); CHECK(!m.penalties[1].crown_restoration_pending&&m.balls[0].crown_mark[0]==2&&count_log(m,"crown_restoration_served")==2);'''),
("certified_match_expires_ordinary_restoration_without_erasing_ejection", r'''auto m=short_match(true); int severe=m.record_penalty(8,"dangerous contact",Severity::Severe); CHECK(m.resolve_penalty(severe,"ejection")&&m.resume()); CHECK(m.crown_exit(0,{0,0,138},1)&&m.crown_return(0)&&m.process_batch(5001,{caught(4)})); CHECK(m.prepare_crown_restart(2)&&m.certify()&&m.status==Status::Complete); CHECK(!m.penalties[1].crown_restoration_pending&&m.players[8].ejected&&m.penalties[0].severity==Severity::Severe&&count_log(m,"crown_restoration_expired")==1);'''),
("simultaneous_crown_returns_stop_once_and_preserve_timeout", r'''Match m(legacy_crown_config()); CHECK(m.process_batch(0,{caught()})); m.advance(177000); CHECK(m.crown_exit(0,{0,0,138})&&m.crown_exit(1,{1,0,138})); m.advance(3000); CHECK(m.status==Status::Paused); for(auto b:m.balls)CHECK(!b.live); CHECK(count_log(m,"stoppage")==1&&m.resume()&&m.balls[3].live);'''),
