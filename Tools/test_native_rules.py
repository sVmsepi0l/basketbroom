...
std::string fingerprint(const Match& m) {
 ...
 for(const auto& b:m.balls) { s<<int(b.type)<<','<<b.phase_active<<','<<b.live<<','<<b.controller<<','<<b.dead_reason<<','<<b.timeout_until<<','<<b.crown_deadline<<','<<b.protection_until<<','<<b.restart_team; for(auto v:b.crown_mark)s<<','<<v; s<<';'; }
 ...
 for(const auto& p:m.penalties) s<<p.id<<','<<p.player<<','<<p.ball<<','<<p.reason<<','<<p.disposition<<','<<int(p.severity)<<','<<p.committed_ms<<','<<p.pending<<';';
