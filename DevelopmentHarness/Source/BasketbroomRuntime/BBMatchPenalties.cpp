...
void ABBMatchState::FinishPenaltyShot(BB::PenaltyShotOutcome Outcome, const FString& Reason, const BB::PointEvent& Goal)
{
    ...
    if (Outcome == BB::PenaltyShotOutcome::Goal && Rules->penalty_shot.awarded_points > 0)
        if (ABBRiderCharacter* Scorer = PenaltyShooterActor.Get())
            if (Scorer == RiderForSlot(PenaltyShooterSlot))
